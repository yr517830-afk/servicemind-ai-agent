...
ServiceMind 支持通过 OpenAI Responses API 将客户消息提取为经过 Pydantic 校验的结构化结果，包含：
...
项目使用 `IntentExtraction` 约束模型输出，并禁止未定义字段。LLM 未配置、调用失败、返回无效结构或客户消息为空时，服务会返回 `other / unknown` 安全兜底结果，避免模型故障影响主业务。
