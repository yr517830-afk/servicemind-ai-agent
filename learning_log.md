...
1. 理解结构化输出相对于自由文本的工程价值。
2. 能够说明 Prompt、Schema 和 Fallback 的不同职责。
3. 结合 ServiceMind 项目整理一份可用于面试表达的技术笔记。

## 完成内容

新增 `docs/structured_output_review.md`，系统整理了以下内容：

- Prompt 负责描述任务、上下文和业务规则。
- Schema 负责约束字段、类型、枚举值和必填项。
- 程序校验负责拒绝系统无法安全处理的结果。
- Fallback 负责将模型故障转换成可执行的业务动作。
- 独立评测集负责发现格式正确但语义错误的预测。

同时对自由文本与结构化输出进行了对比，涵盖程序处理、自动校验、测试、系统集成和失败降级等维度。

## 核心理解

结构化输出的价值不是让模型永远正确，而是把模型的不确定性限制在系统能够检查和处理的范围内。
...
1. Prompt 清晰不代表模型输出一定稳定。
2. Schema 能保证格式和类型，但不能保证语义一定正确。
3. Fallback 让模型失败不会直接中断业务流程。
4. 结构化结果更适合连接 API、数据库和自动化工作流。
5. 评测集是发现语义错误和回归问题的重要工具。

## 面试表达

我在 ServiceMind 中使用 Prompt、Pydantic Schema、异常校验和 Fallback 构建结构化意图提取流程，并通过独立评测集统计意图准确率、风险准确率和错误样本。该方案不能保证模型永远正确，但可以保证输出可校验、可测试、可监控，并在失败时安全降级。

## Day 21 完成情况

- [x] 完成结构化输出技术复盘
- [x] 梳理 Prompt、Schema、校验与 Fallback 的职责
- [x] 对比自由文本与结构化输出
- [x] 准备 2 分钟面试表达
- [ ] 求职投递（按当前学习安排暂缓）
